...
class OrderServices:
    ...
    def get_order(self, order_id):
        ...
        database and returns it. If no order is found returns a string """
    ...
    def get_customer_orders(self, customer_id):
        """ Takes in a customer id and returns a list with all orders 
        for that customer. Returns and empty list if nothing is found. """
        customer_order_list = []
        for order in self.__order_db.get_all_orders():
        ...
        for order in self.__order_db.get_all_orders():
            if order.get_order_id() == order_id:           
                ...
                if order.get_additional_insurance() == "TRUE":                
                    #From the order object, we obtain the registration number 
                    #for the car and send it into get_car_by_regnum to get car category price
    ...
    def get_additional_cost_extra_mileage(self, order_id):
        ...
        for order in self.__order_db.get_all_orders():
            if order.get_order_id() == order_id:                      
                #From the order object, we obtain the registration number for the car 
                #and send it into get_car_by_regnum to get car category price
                car = self.__car_services.get_car(order.get_car_id())                 
                #The cost of additional mileage over 100km is 1% of daily rental cost
    ...
    def get_cost_without_additions(self, order_id):
        ...
        for order in self.__order_db.get_all_orders():
            if order.get_order_id() == order_id:  
                ...
                #From the order object, we obtain the registration number for the 
                #car and send it into get_car_by_regnum to get car category price
                car = self.__car_services.get_car(order.get_car_id())
